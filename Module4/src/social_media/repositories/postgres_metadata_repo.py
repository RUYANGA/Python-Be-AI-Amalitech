...
class PostMetadataRepository:
    ...
    def find_by_id(self, post_id: int) -> dict[str, Any] | None:
        with self._pg.cursor() as cur:
            cur.execute(
                "SELECT metadata FROM post_metadata WHERE post_id = %s", (post_id,)
            )
            row = cur.fetchone()
    ...
    def find_many(self, post_ids: list[int]) -> dict[int, dict]:
        ...
        with self._pg.cursor() as cur:
            ...
            return {row["post_id"]: row["metadata"] for row in cur.fetchall()}

    def delete(self, post_id: int) -> None:
        with self._pg.cursor() as cur:
            cur.execute("DELETE FROM post_metadata WHERE post_id = %s", (post_id,))
