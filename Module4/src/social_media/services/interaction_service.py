...
class LikeService:
    ...
    def _log(
        self, user_id: Any, action: str, target_id: Any, metadata: dict | None = None
    ) -> None:
        self._activity.log(user_id, action, "post", target_id, metadata)

    def like(self, user_id: Any, post_id: Any) -> bool:
        if self._likes.exists(user_id, post_id):
            log.debug(
                "Like skipped (already liked): user %s on post %s", user_id, post_id
            )
            return False
        self._likes.insert(Like(user_id=user_id, post_id=post_id).to_doc())
        self._posts.increment_like_count(post_id, +1)
    ...
    def unlike(self, user_id: Any, post_id: Any) -> bool:
        if self._likes.remove(user_id, post_id):
            self._posts.increment_like_count(post_id, -1)
...
class CommentService:
    ...
    def add(
        ...
        self._posts.increment_comment_count(post_id, +1)
        self._activity.log(
            user_id, "comment", "post", post_id, {"comment_id": str(cid)}
        )
        log.debug("Comment added: %s on post %s by user %s", cid, post_id, user_id)
        doc = self._comments.find_by_id(cid)
        assert doc is not None
        return doc

    def for_post(self, post_id: Any) -> list:
        return self._comments.for_post(post_id)
