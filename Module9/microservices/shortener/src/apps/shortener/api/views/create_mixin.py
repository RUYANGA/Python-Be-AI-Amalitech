...
class URLCreateMixin:
    ...
    Mixed into a view alongside ``BaseURLView`` (which actually sets
    ``self.service``) — this annotation just tells mypy that attribute
    will exist by the time ``post`` runs.
    """
