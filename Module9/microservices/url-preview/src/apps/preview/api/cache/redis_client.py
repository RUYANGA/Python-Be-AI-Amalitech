...
backs both the preview result cache (``preview:cache:*``) and the
domain circuit breaker's state (``preview:circuit:*`` — see
...
class RedisClient:
    ...
    Features:
        ...
        - Graceful degradation: returns ``None``/``0``/``False`` on
          connection failures
    ...
    def incr(self, key: str, ttl: int | None = None) -> int:
        """Increment a counter, returning its new value (``0`` on error).

        Sets the TTL only on the *first* increment (when the returned
        count is ``1``), so it doesn't keep resetting on every call — a
        sliding window that naturally expires ``ttl`` seconds after the
        first failure in a burst, rather than ``ttl`` seconds after the
        *last* one.
        """
        try:
            count = cast(int, self._client.incr(key))
            if count == 1:
                self._client.expire(key, ttl or self._DEFAULT_TTL)
            return count
        except (redis.ConnectionError, redis.TimeoutError) as exc:
