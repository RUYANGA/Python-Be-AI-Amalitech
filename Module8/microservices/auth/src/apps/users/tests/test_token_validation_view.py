...
class TestTokenValidationGetForGateway:
    """The ``GET`` contract nginx's ``auth_request`` uses (see ``/gateway``).

    Header-in, header-out — no JSON body either direction — since
    ``auth_request`` forwards the subrequest's headers but drops its body,
    and only ever looks at the response's status code and headers.
    """
    ...
    def test_rejects_a_request_without_the_internal_token(self, api_client, user, settings):
        ...
        # 403, not 401: the bearer token is a genuine, validly-signed JWT,
        # so this service's own JWTAuthentication (its default
        # authentication class) actually authenticates the request — DRF
        # then reports the permission failure as "authenticated but not
        # permitted" rather than "not authenticated at all". nginx's
        # auth_request module treats both as a valid denial either way
        # (see /gateway's `error_page 401 = @unauthorized` and `error_page
        # 403 = @unauthorized`).
        assert response.status_code == 403

    def test_rejects_a_missing_authorization_header(self, api_client, settings):
        settings.INTERNAL_SERVICE_TOKEN = "shared-secret"
    ...
    def test_rejects_a_garbage_token(self, api_client, settings):
        settings.INTERNAL_SERVICE_TOKEN = "shared-secret"
    ...
    def test_returns_the_claims_as_headers_for_a_valid_token(self, api_client, user, settings):
        settings.INTERNAL_SERVICE_TOKEN = "shared-secret"
        access = AccessToken.for_user(user)
        access["username"] = user.username
        access["is_premium"] = True
        ...
        assert response["X-User-Tier"] == "pro"
        assert response["X-User-Is-Premium"] == "true"
