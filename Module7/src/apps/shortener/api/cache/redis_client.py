...
serialization, and graceful degradation when Redis is unavailable.
"""
...
class RedisClient:
    ...
    def __init__(
        ...
        self._url = url or getattr(settings, "REDIS_URL", "redis://127.0.0.1:6379/0")
    ...
    def exists(self, key: str) -> bool:
        """Check if a key exists. Returns ``False`` on connection error."""
        try:
            return bool(self._client.exists(key))
        except (redis.ConnectionError, redis.TimeoutError) as exc:
            logger.warning("redis.exists_failed key=%s error=%s", key, exc)
            return False
